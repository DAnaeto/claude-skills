...
2. **Findings** — call `ReportFindings` once with the surviving findings ranked most-severe first (empty array if none). Each carries `file`, `line`, `summary`, `failure_scenario`, `category`, and `verdict` (`CONFIRMED` when verified at high/max or you re-checked the code yourself; `PLAUSIBLE` otherwise). Set `level` to the effort used. Do not duplicate the findings as prose text.
